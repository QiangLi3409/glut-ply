...
PlyProperty face_props[] = { /* list of property information for a vertex */
  ...
  char *elem_names[] = { /* list of the kinds of elements in the user's object */
	  "vertex", "face"};
void SavePLYfile(char filename[],unsigned int *triangleIndexList,std::vector<JAMESDATA> drawData,int numTriangleVertices,float thershold)
{
	int nverts = drawData.size();
	Vertex* verts = NULL;
	Face* faces = NULL;
	std::vector<unsigned int *> v_vert_ptrs;
	int i,j;	

	verts = new Vertex[drawData.size()];

	for(i=0;i<drawData.size();i++)
	{
		verts[i].x = drawData[i].x;
		verts[i].y = drawData[i].y;
		verts[i].z = drawData[i].z;


	}
	for (i=0; i<numTriangleVertices; i+=3)  
	{
		   int id1 = triangleIndexList[i];     
		   int id2 = triangleIndexList[i+1];   
		   int id3 = triangleIndexList[i+2];   
		   // comptute disttance 
		   float dist[3];
		   dist[0] = dist[1] = dist[2] =0;

		   dist[0] += (drawData[id1].x-drawData[id2].x)*(drawData[id1].x-drawData[id2].x);
		   dist[0] += (drawData[id1].y-drawData[id2].y)*(drawData[id1].y-drawData[id2].y);
           dist[0] += (drawData[id1].z-drawData[id2].z)*(drawData[id1].z-drawData[id2].z);

		   dist[1] += (drawData[id2].x-drawData[id3].x)*(drawData[id2].x-drawData[id3].x);
		   dist[1] += (drawData[id2].y-drawData[id3].y)*(drawData[id2].y-drawData[id3].y);
           dist[1] += (drawData[id2].z-drawData[id3].z)*(drawData[id2].z-drawData[id3].z);	
		   dist[2] += (drawData[id1].x-drawData[id3].x)*(drawData[id1].x-drawData[id3].x);
		   dist[2] += (drawData[id1].y-drawData[id3].y)*(drawData[id1].y-drawData[id3].y);
           dist[2] += (drawData[id1].z-drawData[id3].z)*(drawData[id1].z-drawData[id3].z);
		   ...
		   // put the pointer
		   v_vert_ptrs.push_back(triangleIndexList+i);

	}
	int nfaces = v_vert_ptrs.size();
	faces = new Face[nfaces];
	 /* create the vertex index lists for the faces */
   for (i = 0; i < nfaces; i++)
   {
	   faces[i].intensity = '\010';
	   faces[i].nverts = 3;
	   faces[i].verts = (int *)v_vert_ptrs[i];
    }
  ...
  /*  enforce the .ply filename extension) */
	 ...
	 float version;
#if 1
	 ply = ply_open_for_writing(filename, 2, elem_names, PLY_ASCII, &version);
#else
  ...
  ply_put_comment (ply, "author: qiang li");
  ply_put_obj_info (ply, "from james data");
